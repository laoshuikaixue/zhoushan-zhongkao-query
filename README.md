...
- **录取查询**：录取学校、录取专业、毕业学校
- **录取身份核验**：当录取结果为"舟山市六横中学"时，自动生成 30 字符以内的入群校验码，并展示新生交流群二维码
...
3. 生成固定 **30 字符** 的校验码（适配微信入群申请 ≤30 字符的限制）
4. 考生复制校验码并填入微信入群申请信息

管理员在本地使用 `decrypt.py` 输入校验码即可解密出准考证号、出生年月日和考生姓名。

> 由于微信申请信息限制为 30 字符以内，完整姓名写入仅支持最多 4 个常用汉字。超出该范围或包含生僻字时，页面会提示校验码生成失败。
